...
public class EditRequirements_Activity extends AppCompatActivity {
    ...
    private void setDefault() {
        ...
        if (Utilities.isNetworkAvailable(context)) {
            new GetMainCategotyList().execute("1");
        } else {
        ...
        edt_category.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mainCategoryList.size() == 0) {
                    if (Utilities.isNetworkAvailable(context)) {
                        new GetMainCategotyList().execute("2");
                    } else {
                        Utilities.showMessage(R.string.msgt_nointernetconnection, context, 2);
                    }
                } else {
    ...
    private void submitData() {
        ...
        mainObj.addProperty("type", "updaterequirement");
        ...
        if (Utilities.isNetworkAvailable(context)) {
            new EditRequirement().execute(mainObj.toString().replace("\'", Matcher.quoteReplacement("\\\'")));
        } else {
            Utilities.showMessage(R.string.msgt_nointernetconnection, context, 2);
        }
                        ...
                        if (TYPE.equals("1")) {
                            for (MainCategoryListModel catDetails : mainCategoryList) {
                                if (catDetails.getId().equals(reqDetails.getCategory_type_id())) {
                                    edt_category.setText(catDetails.getType_description());
                                }
                            }
    ...
    private class EditRequirement extends AsyncTask<String, Void, String> {
            ...
            try {
                ...
                if (!result.equals("")) {
                    ...
                    if (type.equalsIgnoreCase("success")) {
                        ...
                        tv_title.setText("Requirement updated successfully");
