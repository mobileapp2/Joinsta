...
public class EditRatingAndReview_Activity extends AppCompatActivity {
    ...
    private RatingAndReviewModel.ResultBean ratingDetails;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_edit_rating_and_review);
    ...
    private void init() {
        context = EditRatingAndReview_Activity.this;
    ...
    private void setDefault() {
        ratingDetails = (RatingAndReviewModel.ResultBean) getIntent().getSerializableExtra("ratingDetails");

        edtPublicName.setText(ratingDetails.getPublic_name());
        edtTitle.setText(ratingDetails.getReview_title());
        edtReview.setText(ratingDetails.getReview_description());
        smileRating.setSelectedSmile(Integer.parseInt(ratingDetails.getRating()) - 1);
...
//            Utilities.showMessage(R.string.msgt_nointernetconnection, context, 2);
    ...
    private void submitData() {
        ...
        mainObj.addProperty("type", "UpdateProfileRating");
        mainObj.addProperty("profile_reviews_ratings_id", ratingDetails.getId());
        mainObj.addProperty("category_type_id", ratingDetails.getCategory_type_id());
        mainObj.addProperty("record_id", ratingDetails.getRecord_id());
        ...
        if (Utilities.isNetworkAvailable(context)) {
            new EditRatingAndReview().execute(mainObj.toString().replace("\'", Matcher.quoteReplacement("\\\'")));
        } else {
            Utilities.showMessage(R.string.msgt_nointernetconnection, context, 2);
        }
    }

    private class EditRatingAndReview extends AsyncTask<String, Void, String> {
            ...
            try {
                ...
                if (!result.equals("")) {
                    ...
                    if (type.equalsIgnoreCase("success")) {
                        LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("GroupMembersProfileDetails_Activity"));
                        LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("RatingAndReviewList_Activity"));
                        new Search_Fragment.GetSearchList().execute(session.getLocation().get(ApplicationConstants.KEY_LOCATION_INFO));

                        switch (ratingDetails.getCategory_type_id()) {
                            case "1":
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewSearchBizDetails_Activity"));
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewGroupMemberBizDetails_Activity"));
                                break;
                            case "2":
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewSearchEmpDetails_Activity"));
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewGroupMemberEmpDetails_Activity"));
                                break;
                            case "3":
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewSearchProfDetails_Activity"));
                                LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("ViewGroupMemberProfDetails_Activity"));
                                break;
                        }
                        ...
                        tv_title.setText("Your review is updated successfully, thank you!");
