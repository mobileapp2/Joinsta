...
public class ViewGroupMemberEmpDetails_Activity extends AppCompatActivity {
    ...
    private void setDefault() {
        ...
        IntentFilter intentFilter = new IntentFilter("ViewGroupMemberEmpDetails_Activity");
...
//                        holder.tv_distance.setText(Html.fromHtml("<font color=\"#FFA000\"> <b>" + time_distance[0] + "</b></font> <font color=\"#616161\">from current location</font>"));
    ...
    private void showLandlineListDialog(final ArrayList<GetEmployeeModel.ResultBean.LandlineBean> landlineList) {
        ...
        for (int i = 0; i < landlineList.size(); i++) {
            arrayAdapter.add(landlineList.get(i).getCountry_code() + landlineList.get(i).getLandline_numbers());
        }
        ...
        builderSingle.setAdapter(arrayAdapter, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                startActivity(new Intent(Intent.ACTION_CALL,
                        Uri.parse("tel:" + landlineList.get(which).getCountry_code() + landlineList.get(which).getLandline_numbers())));
            }
        });
        builderSingle.show();
    }
